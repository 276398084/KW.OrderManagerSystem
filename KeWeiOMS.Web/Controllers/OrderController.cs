...
namespace KeWeiOMS.Web.Controllers
{
    ...
    public class OrderController : BaseController
    {
        ...
        public ActionResult EditOrderVali()
        {
        ...
        public ActionResult ScanExport(DateTime st, DateTime et, string u, string key)
        {
            ...
            string sql = "select OrderNo as 'PackageNo',OrderExNo as 'OrderExNo',Weight as 'PackageWeight',ScanningBy,TrackCode as 'TrackCode',ScanningOn as 'ShippedTime',LogisticMode as 'LogisticsMode',Country from Orders where Status in ('已发货','已完成') and {0}  ScanningOn  between '{1}' and '{2}' {3}  order by ScanningOn asc ";
        ...
        [HttpPost]
        public JsonResult EditReOrder(string o, string rows)
        {
            NSession.Clear();
            OrderType obj = GetById(Utilities.ToInt(o));
            obj.IsRepeat = 0;
            ...
            if (o.Platform == PlatformEnum.B2C.ToString())
            {
                try
                {
                            ...
                            new SqlConnection("server=97.74.123.157;database=FeiduGS;uid=sa;pwd=`1q2w3e4r"))
                    {
                        ...
                        connection.Close();
                        connection.Dispose();
                        new System.Net.WebClient().DownloadString("http://sendmail.gamesalor.com.cn/SendMailMessage.ashx?txnId=" + o.OrderExNo + "&ems=" + o.TrackCode + "&mail=" + o.BuyerEmail);
                    }
                }
                catch (Exception)
                {
        ...
        public JsonResult GetOrderByBeforePei(string orderNo)
        {
            ...
            if (orders.Count > 0)
            {
                ...
                if (order.Status == OrderStatusEnum.已处理.ToString())
                {
                    ...
                    string html2 = @"<tr style='font-weight:bold; font-size:30px;' name='tr_{0}' code='{3}' qty='{1}' cqty='{4}'><td><img width='220px' src='/imgs/pic/{0}/1.jpg' /></td><td>{0}*{1}</td><td>{2}</td><td><span></td></tr>";
                    order.Products =
                        NSession.CreateQuery("from OrderProductType where OId=" + order.Id).List<OrderProductType>().ToList();
                    foreach (var p in order.Products)
                    {
